 ...
 * @returns {Promise<Object>} Response
 */
const getTimetable = async (sectionId) => {
  try {
    const response = await api.get(`/academics/timetable/section/${sectionId}`);
    return response;
  } catch (error) {
    return error.response;
  }
...
const getDivisions = async (standard) => {
  try {
    const response = await api.get('/academics/divisions', { params: { standard } });
    return response;
  } catch (error) {
    return error.response;
  }
...
const assignClassTeacher = async (sectionId, teacherId) => {
  try {
    const response = await api.put(`/academics/sections/${sectionId}/class-teacher/${teacherId}`);
    return response;
  } catch (error) {
    return error.response;
  }

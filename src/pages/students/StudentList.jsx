...
export default function StudentList() {
  ...
  const openSingleUpdateModal = (studentId) => {
    ...
    if (student && student.sectionId) setTargetSection(student.sectionId);
    else setTargetSection("");
  ...
  const openPhotoModal = (student) => {
    ...
    setPhotoPreview(
      student.profilePicture
        ? `${import.meta.env.VITE_API_URL}/${student.profilePicture}`
        : "",
    );
    onPhotoOpen();
  };
  ...
  const handlePhotoFileChange = (e) => {
    ...
    if (file) {
      ...
      if (!["image/jpeg", "image/png"].includes(file.type)) {
        addToast({
          title: "Error",
          description: "Only JPG and PNG are allowed",
          color: "danger",
        });
        return;
      }
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      const response = await studentService.uploadProfilePicture(
        photoStudent.id,
        formData,
      );
      if (response.data?.success) {
  ...
  const handleDownloadIDCard = async (student) => {
    ...
    try {
      ...
      if (result.success) {
        addToast({
          title: "Success",
          description: "ID Card downloaded",
          color: "success",
        });
      } else {
        addToast({
          ...
          action={
            <div className="flex gap-2">
              {(selectedKeys === "all" || selectedKeys.size > 0) && (
                ...
                >
                  Move / Promote (
                  {selectedKeys === "all" ? students.length : selectedKeys.size}
                  )
                </Button>
              )}
              <Button
                ...
                onPress={() =>
                  (window.location.href = "/admin/bulk-upload/students")
                }
              >
                Bulk Upload
              </Button>
      ...
      <motion.div variants={itemVariants}>
        <Card className="bg-content1 border border-default-200 shadow-sm">
          ...
          <CardBody className="px-0 pb-4 overflow-x-auto">
            ...
            >
              <TableHeader>
                <TableColumn className="bg-default-100 text-default-500 font-semibold">
                  STUDENT
                </TableColumn>
                <TableColumn className="bg-default-100 text-default-500 font-semibold">
                  ADMISSION NO
                ...
                {students.map((student) => {
                  ...
                  return (
                    <TableRow key={student.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">{/* Your buttons */}</div>
                      </TableCell>
                    </TableRow>
                  );
                })}
            ...
            <>
              ...
              <ModalBody>
                ...
                <Select
                  ...
                  selectedKeys={targetSection ? [targetSection.id] : []}
                  onChange={(e) => {

...
const CreateDailyTimetable = () => {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (staffRes?.data?.success) {
        const allStaff = Array.isArray(staffRes.data?.data)
          ? staffRes.data?.data
          : [];
    ...
    try {
      ...
      if (response?.data?.success) {
        addToast({
          title: "Success",
          description:
            response.data.message || "Daily Timetable Created Successfully",
          color: "success",
        });
      ...
      const errorMsg =
        ...
        "An error occurred while creating the timetable";
      setError(errorMsg);
    } finally {
      setSaving(false);
    }
  ...
  const renderStep1 = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        <label className="text-sm font-medium mb-1.5 block text-default-700">
          Class/Section
        </label>
        <Select
          ...
          selectedKeys={
            formData.sectionId ? new Set([formData.sectionId]) : new Set()
          }
          variant="bordered"
        >
          {sections.map((s) => (
      ...
      <div>
        ...
        <Input
          ...
          onBlur={(e) =>
            handleSelectionChange("periodCount", new Set([e.target.value]))
          }
          variant="bordered"
        />
      </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-default-500 mb-4">
          Define the start and end time for each period.
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {formData.periods.map((period, index) => (
          <Card
            key={index}
            className="bg-default-50 border border-default-200 shadow-sm"
          >
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-default-500 mb-4">
          Assign a subject and teacher for each period.
        </p>
      </div>
      <div className="grid grid-cols-1 gap-4">
        {formData.periods.map((period, index) => (
          <div
            key={index}
            className="p-4 border rounded-xl hover:shadow-md transition-shadow bg-content1 border-default-200"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Select
                  ...
                  selectedKeys={
                    period.subjectId
                      ? new Set([String(period.subjectId)])
                      : new Set()
                  }
                  onSelectionChange={(keys) =>
                    handlePeriodChange(index, "subjectId", Array.from(keys)[0])
                  }
                  variant="bordered"
                >
                  {subjects.map((s) => {
              ...
              <div>
                ...
                <Select
                  ...
                  selectedKeys={
                    period.teacherId
                      ? new Set([String(period.teacherId)])
                      : new Set()
                  }
                  onSelectionChange={(keys) =>
                    handlePeriodChange(index, "teacherId", Array.from(keys)[0])
                  }
                  variant="bordered"
                >
                  {teachers.map((t) => (
                    <SelectItem
                      key={String(t.user?.id)}
                      textValue={`${t.user?.name} - ${t.staffProfile?.designation || t.staffProfile?.workingAs || t.designation || t.workingAs || "Teacher"}`}
                    >
                      <div className="flex flex-col">
                        <span className="font-medium">{t.user?.name}</span>
                        <span className="text-tiny text-default-500">
                          {t.staffProfile?.designation ||
                            t.staffProfile?.workingAs ||
                            t.designation ||
                            t.workingAs ||
                            "Teacher"}
                        </span>
              ...
              <div>
                ...
                <Input
                  ...
                  value={period.room}
                  onChange={(e) =>
                    handlePeriodChange(index, "room", e.target.value)
                  }
    ...
    >
      <div className="flex items-center gap-4 mb-6">
        <Button
          isIconOnly
          variant="light"
          onPress={() => navigate("/academic/timetable")}
        >
        ...
        <div>
          ...
          <p className="text-sm text-default-500">
            Configure schedule for a specific class and day
          </p>
        </div>
      </div>
        ...
        <div className="p-6 border-t border-default-200 flex justify-end gap-3 bg-default-50/50">
          ...
          {step < 3 ? (
            <Button
              ...
              isDisabled={
                step === 1 &&
                (!formData.sectionId || !formData.day || !formData.periodCount)
              }
            >
              Next
            </Button>
